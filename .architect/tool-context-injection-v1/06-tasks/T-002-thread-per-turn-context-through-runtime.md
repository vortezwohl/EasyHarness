# Task: T-002-thread-per-turn-context-through-runtime
...
- Design Rule References: R-D002-001, R-D002-002
...
## Functional Boundary
- Requested Functionality: Accept named Context values on run and stream and pass a fresh private map through Strands invocation state.
- Protected Functionality: Old prompt-only API, session reuse, cancellation, stream order, and per-invocation isolation.
...
- Mandatory Stop Condition: A per-call invocation-state route cannot be used without storing Context on Agent, Tool, or module state.

## Code Impact Scope
| Likely Code Location | Relevant Symbol or Area | Expected Work | Evidence or Rationale |
| --- | --- | --- | --- |
| easyharness/_internal/runtime.py | Relevant SDK seam | Make the smallest approved change | Approved D-design evidence; scope may expand only by recorded evidence. |
...
## MUST DO
- M-T002-001: Implement only the approved task boundary and preserve ordinary-tool behavior.

## MUST NOT DO
- N-T002-001: Do not introduce global, Agent-held, Tool-held, or publicly observable Context state.
