# Task: T-001-add-tool-context-contract
...
- Design Rule References: R-D001-001, R-D001-002
...
## Functional Boundary
- Requested Functionality: Define and export ToolContext; classify annotated hidden parameters and build schemas only from ordinary parameters.
- Protected Functionality: Strict metadata, Pydantic ordinary input validation, zero-argument tools, direct Python calls.
...
- Mandatory Stop Condition: The repository requires treating ordinary parameters as hidden or changes schemas for tools without Context.

## Code Impact Scope
| Likely Code Location | Relevant Symbol or Area | Expected Work | Evidence or Rationale |
| --- | --- | --- | --- |
| easyharness/_internal/types.py and easyharness/_internal/tools.py | Relevant SDK seam | Make the smallest approved change | Approved D-design evidence; scope may expand only by recorded evidence. |
...
## MUST DO
- M-T001-001: Implement only the approved task boundary and preserve ordinary-tool behavior.

## MUST NOT DO
- N-T001-001: Do not introduce global, Agent-held, Tool-held, or publicly observable Context state.
